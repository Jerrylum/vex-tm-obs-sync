...
from vex_tm_bridge.base import (
...
class OBSTMSync:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.config = config
        self.obs_client: Optional[ReqClient] = None
        self.obs_event_client: Optional[EventClient] = None
        self.bridge_engine: Optional[BridgeEngine] = None
        self.fieldset: Optional[Fieldset] = None
        self.running = False

        # Create mapping dictionaries for quick lookups
        self.field_scenes: List[str] = [
            mapping.obs_scene for mapping in config.field_scene_mappings
        ]
        self.obs_to_tm: Dict[str, str] = {
            mapping.obs_scene: mapping.tm_display
            for mapping in config.other_scene_mappings
        }
        self.tm_to_obs: Dict[str, str] = {
            mapping.tm_display: mapping.obs_scene
            for mapping in config.other_scene_mappings
        }
    ...
    def connect_obs(self) -> None:
        ...
        try:
            logger.info(
                f"Connecting to OBS at {self.config.obs.host}:{self.config.obs.port}"
            )
            self.obs_client = ReqClient(
                host=self.config.obs.host,
                port=self.config.obs.port,
                password=self.config.obs.password,
            )
            self.obs_event_client = EventClient(
                host=self.config.obs.host,
                port=self.config.obs.port,
                password=self.config.obs.password,
            )
            ...
            logger.info(f"Connected to OBS {version_info.obs_version} (WebSocket {version_info.obs_web_socket_version})")  # type: ignore

            # Get current scene
            current_scene = self.obs_client.get_current_program_scene()
            self.last_obs_scene = current_scene.scene_name  # type: ignore
            ...
            if self.config.sync_obs_to_tm:
                ...
                def on_current_program_scene_changed(event_data: Any) -> None:
                    self._on_obs_scene_changed(event_data)

                self.obs_event_client.callback.register(
    ...
    def connect_vex_tm(self) -> None:
        ...
        try:
            ...
            self.bridge_engine = get_bridge_engine(
                self.config.vex_tm.competition, low_cpu_usage=True
            )
            self.bridge_engine.start()

            # Get fieldset
            logger.info(f"Getting fieldset: {self.config.vex_tm.fieldset_title}")
            self.fieldset = self.bridge_engine.get_fieldset(
                self.config.vex_tm.fieldset_title
            )
    ...
    def _on_obs_scene_changed(self, event_data: Any) -> None:
        """Handle OBS scene change events."""
        if not self.config.sync_obs_to_tm:
            return

        logger.debug(f"OBS scene changed event: {event_data}")
            ...
            if scene_name in self.field_scenes:
                ...
                try:
                    if self.fieldset is None:
                        logger.error("Fieldset is not connected")
                        return
                    ...
                    logger.info(
                        f"Syncing field scene '{scene_name}' -> TM display '{tm_display_name}' (match state: {match_state})"
                    )
                    ...
                    logger.info(
                        f"Successfully updated TM audience display to: {tm_display_name}"
                    )

                except Exception as e:
                    logger.error(
                        f"Failed to update TM audience display for field scene: {e}"
                    )
                ...
                logger.info(
                    f"Syncing OBS scene '{scene_name}' -> TM display '{tm_display_name}'"
                )
    ...
    def _on_tm_overview_updated(
        self, fieldset: Fieldset, overview: FieldsetOverview
    ) -> None:
        ...
        with self._sync_lock:
            ...
            if audience_display_name in [
                FieldsetAudienceDisplay.Intro.name,
                FieldsetAudienceDisplay.InMatch.name,
            ]:
                # For field-related displays, switch to the field scene based on current field ID
                try:
                    current_field_id = overview.current_field_id
                    if current_field_id is not None and 0 <= current_field_id < len(
                        self.field_scenes
                    ):
                        obs_scene = self.field_scenes[current_field_id]
                        logger.info(
                            f"Syncing TM display '{audience_display_name}' -> Field scene '{obs_scene}' (field {current_field_id})"
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                f"Successfully updated OBS scene to: {obs_scene}"
                            )
                        except Exception as e:
                            logger.error(f"Failed to update OBS scene: {e}")
                    else:
                        logger.warning(
                            f"Invalid or missing field ID: {current_field_id}, cannot map to field scene"
                        )
                ...
                logger.info(
                    f"Syncing TM display '{audience_display_name}' -> OBS scene '{obs_scene}'"
                )
                ...
                logger.debug(
                    f"No mapping found for TM audience display: {audience_display_name}"
                )
            ...
            if self.config.sync_obs_to_tm and self.last_obs_scene:
                ...
                if self.last_obs_scene in self.field_scenes:
                    try:
                        ...
                        target_display = (
                            FieldsetAudienceDisplay.Intro.name
                            if match_state == FieldsetState.Disabled
                            else FieldsetAudienceDisplay.InMatch.name
                        )
                        if target_display != self.last_tm_display:
                            logger.info(
                                f"Initial sync: Field scene '{self.last_obs_scene}' -> TM display '{target_display}'"
                            )
                ...
                # Handle other scenes
                    ...
                    if tm_display_name != self.last_tm_display:
                        logger.info(
                            f"Initial sync: OBS scene '{self.last_obs_scene}' -> TM display '{tm_display_name}'"
                        )
                        try:
                            if self.fieldset is None:
                                logger.error("Fieldset is not connected")
                            ...
                            tm_display = FieldsetAudienceDisplay.by_name(
                                tm_display_name
                            )
                            self.fieldset.set_audience_display(tm_display)
                            self.last_tm_display = tm_display_name
                ...
                if self.last_tm_display in [
                    ...
                    try:
                        ...
                        if (
                            ...
                            and 0 <= current_field_id < len(self.field_scenes)
                        ):
                            obs_scene = self.field_scenes[current_field_id]
                            if obs_scene != self.last_obs_scene:
                                logger.info(
                                    f"Initial sync: TM display '{self.last_tm_display}' -> Field scene '{obs_scene}' (field {current_field_id})"
                                )
                                if self.obs_client is None:
                                    logger.error("OBS client is not connected")
                                    return
                ...
                # Handle other displays
                    ...
                    if obs_scene != self.last_obs_scene:
                        logger.info(
                            f"Initial sync: TM display '{self.last_tm_display}' -> OBS scene '{obs_scene}'"
                        )
                        try:
                            if self.obs_client is None:
                                logger.error("OBS client is not connected")
