...
def load_config(config_path: Path) -> Config:
    ...
    try:
        ...
        raise ValueError(f"Invalid competition type: {competition_str}. Must be one of: {list(Competition.__members__.keys())}")
    ...
    for mapping in other_mappings_data:
        ...
        if not obs_scene or not tm_display:
            raise ValueError(f"Other scene mapping must have both 'obs_scene' and 'tm_display': {mapping}")
        
        other_scene_mappings.append(OtherSceneMapping(obs_scene=obs_scene, tm_display=tm_display))
...
def validate_config(config: Config) -> None:
    ...
    if not config.field_scene_mappings and not config.other_scene_mappings:
        raise ValueError("At least one scene mapping (field or other) must be configured")
    ...
    for mapping in config.field_scene_mappings:
        ...
        if mapping.obs_scene in field_obs_scenes:
            raise ValueError(f"Duplicate field OBS scene in mappings: {mapping.obs_scene}")
    ...
    for mapping in config.other_scene_mappings:
        ...
        if mapping.obs_scene in other_obs_scenes:
            raise ValueError(f"Duplicate other OBS scene in mappings: {mapping.obs_scene}")
    ...
    if overlap:
        raise ValueError(f"OBS scenes cannot be in both field and other mappings: {overlap}")
    ...
    for mapping in config.other_scene_mappings:
        if mapping.tm_display not in valid_displays:
            raise ValueError(f"Invalid TM display '{mapping.tm_display}'. Valid options: {sorted(valid_displays)}")
